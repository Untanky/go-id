...
func (suite *LoginTestSuite) SetupTest() {
	...
	suite.service.KnownUsers = []*User{{Identifier: knownUserId + "0", Passkey: knownUserKey + "0"}, {Identifier: knownUserId + "1", Passkey: knownUserKey + "1"}}
}

func (suite *LoginTestSuite) TestLogin_LoginWithKnownUser() {
	var err error

	err = suite.service.Login(suite.service.KnownUsers[0].Identifier, suite.service.KnownUsers[0].Passkey)
	assert.Nil(suite.T(), err)

	err = suite.service.Login(suite.service.KnownUsers[1].Identifier, suite.service.KnownUsers[1].Passkey)
	assert.Nil(suite.T(), err)
}
...
func (suite *LoginTestSuite) TestLogin_LoginWithKnownUserAndIncorrectPasskey() {
	...
	err = suite.service.Login(suite.service.KnownUsers[0].Identifier, suite.service.KnownUsers[1].Passkey)
	assert.ErrorContains(suite.T(), err, "unauthorized")

	err = suite.service.Login(suite.service.KnownUsers[1].Identifier, suite.service.KnownUsers[0].Passkey)
	assert.ErrorContains(suite.T(), err, "unauthorized")

	err = suite.service.Login(suite.service.KnownUsers[1].Identifier, "foo")
	assert.ErrorContains(suite.T(), err, "unauthorized")
}
...
func (suite *RegisterTestSuite) TestRegister_KnownUserShouldContainNewUser() {
	...
	assert.Nil(suite.T(), err)
	err = suite.service.Register(user2)
	assert.Nil(suite.T(), err)

	assert.Len(suite.T(), suite.service.KnownUsers, 2)
	assert.Equal(suite.T(), suite.service.KnownUsers[0], user1)
	assert.Equal(suite.T(), suite.service.KnownUsers[1], user2)
}
